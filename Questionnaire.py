...
def closeDialogAndReset(dialog):
    """
    Close the dialog and reset the flag.
    """
    self.flag = True  # Reset the flag
...
class GenderQuestion(Frame):
    ...
    def __init__(self, master, controller):
        Frame.__init__(self, master)
        self.controller = controller
        self.flag = True  # Define flag as an instance variable
    ...
    def nextQuestion(self):
        ...
        if answer == '0':
            if self.flag:  # Only show the dialog box if no dialog is currently open
                self.flag = False  # Set flag to False to prevent multiple dialogs
                dialogBox("ไม่ได้กรอกแบบสอบถาม", "คุณยังไม่ได้กรอกแบบสอบถาม\nโปรดกรอกแบบสอบถาม")
                self.flag = True  # Reset flag when the dialog is closed
        else:
