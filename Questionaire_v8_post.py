import sys
from tkinter import (Tk, Text, Label, ttk, Button, Radiobutton, Frame, Menu,
...
def finishedDialog(title, message):
    ...
    """
    dialog = Toplevel()  # Fix: Use Toplevel() instead of Tk()
    dialog.wm_title(title)
    dialog.grab_set()
    ...
    label = Label(dialog, text=message, font=("Sarabun", 64, "bold"), wraplength=1200, justify="center", bg="white")
    label.pack(side="top", fill="x", pady=80)

    # Create Exit Button
    exit_button = Button(dialog, text="เสร็จสิ้น", font=("Sarabun", 36), bg="red", fg="white",
                         padx=40, pady=20, command=lambda: quit_application(dialog))
    exit_button.pack(pady=50)

    # OK button with larger font
    ok_button = ttk.Button(dialog, text="จบแบบสอบถาม", command=lambda: quit_application(dialog), style="Custom.TButton")
    ok_button.pack(ipady=20, pady=40)

    # Configure button style
    s = ttk.Style()
    s.configure("Custom.TButton", font=("Sarabun", 30))

    # Prevent ALT + F4 from closing the dialog
    dialog.protocol("WM_DELETE_WINDOW", disable_event)

def quit_application(dialog):
    """
    Close all Tkinter windows and force quit the application.
    """
    dialog.destroy()  # Close the final dialog window

    try:
        app.quit()  # Quit the main Tkinter loop
        app.destroy()  # Destroy the main application window
    except NameError:
        print("Main application already closed.")
...
class StartPage(Frame):
    ...
    def __init__(self, master, controller):
        ...
        info_text = (
            "แบบประเมินทั้งหมดประกอบด้วย\n"
            "แบบประเมินทางจิตวิทยา ประกอบด้วย\n"
        ...
        info_label.pack(pady=10, padx=(30, 10), ipadx=20, ipady=3, fill="x")
        ...
        purpose_label = Label(self, text=purpose_text, font=("Sarabun", 20), borderwidth=2, relief="ridge",
                              anchor="w", justify="left")
        purpose_label.pack(pady=10, padx=(30, 10), ipadx=5, ipady=3, fill="x")
        ...
        start_button = ttk.Button(self, text="เริ่มต้นแบบสอบถาม",
                                  command=lambda: controller.show_frame(MindfulAttentionAwarenessScale),
...
class GenderQuestion(Frame):
    ...
    def __init__(self, master, controller):
        ...
        # Next question button with font
    ...
    def nextQuestion(self):
        ...
        and display the next question or frame.
        '''
        answer = self.var.get()
...
class MindfulnessExperience(ttk.Frame):
    ...
    def save_and_continue(self):
        ...
        if self.still_practicing_var.get() == 1:
            self.controller.show_frame(MindfulnessExperienceDetail)
        else:
...
class MindfulnessExperienceDetail(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        ttk.Label(daily_frame, text="ครั้ง/วัน     เฉลี่ย").pack(side='left', padx=(5, 15))
        ttk.Entry(daily_frame, textvariable=self.daily_duration, width=10).pack(side='left', padx=(15, 5))
        ttk.Label(daily_frame, text="นาที/ครั้ง").pack(side='left', padx=(5, 0))
        ...
        ttk.Label(weekly_frame, text="ครั้ง/สัปดาห์     เฉลี่ย").pack(side='left', padx=(5, 15))
        ttk.Entry(weekly_frame, textvariable=self.weekly_duration, width=10).pack(side='left', padx=(15, 5))
        ttk.Label(weekly_frame, text="นาที/ครั้ง").pack(side='left', padx=(5, 0))
        ...
        ttk.Label(monthly_frame, text="ครั้ง/เดือน     เฉลี่ย").pack(side='left', padx=(5, 15))
        ttk.Entry(monthly_frame, textvariable=self.monthly_duration, width=10).pack(side='left', padx=(15, 5))
        ttk.Label(monthly_frame, text="นาที/ครั้ง").pack(side='left', padx=(5, 0))
        ...
        ttk.Label(yearly_frame, text="ครั้ง/ปี     เฉลี่ย").pack(side='left', padx=(5, 15))
        ttk.Entry(yearly_frame, textvariable=self.yearly_duration, width=10).pack(side='left', padx=(15, 5))
        ttk.Label(yearly_frame, text="นาที/ครั้ง").pack(side='left', padx=(5, 0))
    ...
    def save_and_proceed(self):
        ...
        general_answers_list[-1].update(additional_response)
        self.controller.show_frame(MindfulAttentionAwarenessScale)
...
class MindfulAttentionAwarenessScale(Frame):
    def __init__(self, master, controller):
        ...
        self.questions = [
            ...
            "ฉันขับขี่รถไปที่ต่าง ๆ แบบอัตโนมัติ แล้วก็สงสัยว่าฉันไปที่นั่นทำไม",
        ...
        for col, (text, value) in enumerate(scale_values):
            ...
            radio.grid(row=1, column=col, padx=30, pady=20)
...
class FFMQ_SF(Frame):
    def __init__(self, master, controller):
        ...
        frame = Frame(self, borderwidth=2, relief="ridge", bg=original_bg)
        frame.pack(padx=10, pady=10)

        text_widget = Text(frame, font=("Sarabun", 20), wrap="word", height=2, width=50, borderwidth=0,
                           bg=original_bg, fg="black", highlightthickness=0)
        text_widget.pack(padx=10, pady=10)

        text_widget.insert("insert", "โปรดอ่านข้อความ แล้วเลือกตัวเลขที่ตรงกับความเป็นจริงของท่านที่สุด ")
        text_widget.insert("insert", "ณ ขณะนี้", "highlight")
        text_widget.tag_configure("highlight", font=("Sarabun", 20, "bold"), foreground="red")
...
class PHLMS(Frame):
    def __init__(self, master, controller):
        ...
        ttk.Label(self, text="คำชี้แจง : โปรดอ่านข้อความ แล้วเลือกตัวเลขที่ตรงกับความเป็นจริงของท่านที่สุด ณ ขณะนี้", font=custom_font,
                  borderwidth=2, relief="ridge").pack(padx=10, pady=10)
        Label(self, text=" ", font=("Sarabun", 20)).pack(pady=10)
...
class RuminativeThinkingScale(Frame):
    def __init__(self, master, controller):
        ...
        self.questions = [
                    ...
                    "คิดว่า ฉันรู้สึกนิ่งเฉยและขาดแรงจูงใจเพียงใด",
                    "วิเคราะห์เหตุการณ์ในช่วงนี้เพื่อพยายามเข้าใจว่าทำไมฉันจึงรู้สึกหม่นหมอง",
                    "คิดว่า ฉันไม่มีความรู้สึกใดๆ อีกต่อไป",
                    "คิดว่า “ทำไมฉันจึงเริ่มต้นไม่ได้?",
                    "คิดว่า “ทำไมฉันมักจะทำแบบนี้เสมอ?",
                    "ออกมาอยู่กับตัวเองและคิดว่าทำไมฉันจึงรู้สึกแบบนี้",
                    "เขียนสิ่งที่ฉันกำลังคิดและลองวิเคราะห์ดู",
                    ...
                    "คิดว่า ฉันเศร้ามากแค่ไหน",
                    "คิดถึงความบกพร่อง ความล้มเหลว ความผิดพลาดต่าง ๆ ของฉัน",
                    "คิดว่าฉันไม่มีกำลังใจพอที่จะทำอะไรเลย",
                    "วิเคราะห์บุคลิกภาพของตัวเองเพื่อพยายามเข้าใจว่าทำไมฉันจึงรู้สึกซึมเศร้า",
                    "ไปที่สักแห่งเพียงลำพังเพื่อครุ่นคิดถึงความรู้สึกของฉัน",
                    "คิดว่าฉันโกรธตัวเองมากเพียงใด"
                ]
...
class PerceivedStress(Frame):
    def __init__(self, master, controller):
        ...
        ttk.Label(self, text=(
            "ข้อคำถามต่อไปนี้เป็นคำถามเกี่ยวกับความรู้สึกและความคิดของท่าน ณ ขณะนี้ \n"
        ...
        self.questions = [
            "ณ ขณะนี้ฉันรู้สึกผิดหวังหรือไม่สบายใจ เนื่องจากบางสิ่งบางอย่างเกิดขึ้นอย่างไม่คาดคิด",
            "ณ ขณะนี้เหตุการณ์สำคัญที่ฉันเผชิญอยู่ ฉันไม่สามารถเปลี่ยนแปลงหรือจัดการได้",
            "ณ ขณะนี้ฉันรู้สึกเครียดและกระวนกระวายใจ",
            "ณ ขณะนี้ฉันรู้สึกว่าฉันสามารถแก้ไขปัญหาที่ทำให้ฉันไม่สบายใจในชีวิตประจำวันได้สำเร็จ",
            "ณ ขณะนี้ฉันรู้สึกว่าฉันสามารถจัดการกับปัญหาที่สำคัญในชีวิตได้สำเร็จ",
            "ณ ขณะนี้ฉันมั่นใจว่าฉันสามารถแก้ไขหรือจัดการปัญหาได้",
            "ณ ขณะนี้ฉันรู้สึกว่าเหตุการณ์ต่าง ๆ ที่เกิดขึ้นเป็นไปตามที่ฉันต้องการให้เป็นไป",
            "ณ ขณะนี้ฉันพบว่าตนเองไม่สามารถทำงานได้ทันเวลา",
            "ณ ขณะนี้ฉันสามารถกำจัดหรือลดความไม่สบายใจที่เกิดขึ้นได้",
            "ณ ขณะนี้ฉันสามารถเอาชนะปัญหาหรืออุปสรรคที่เกิดขึ้นได้",
            "ณ ขณะนี้ฉันรู้สึกโกรธหรือไม่พอใจที่ตนเองไม่สามารถจัดการกับเหตุการณ์บางอย่างที่เกิดขึ้นได้",
            "ณ ขณะนี้ฉันรู้สึกว่าฉันหมกมุ่นครุ่นคิดอยู่กับเรื่องใดเรื่องหนึ่งเพียงอย่างเดียว",
            "ณ ขณะนี้ฉันสามารถตัดสินใจได้ว่าช่วงเวลาใดฉันสามารถจะทำอะไรได้ด้วยตนเอง",
            "ณ ขณะนี้ฉันรู้สึกเป็นทุกข์ เนื่องจากปริมาณงานต่าง ๆ มีมากจนไม่สามารถจัดการได้หมด"
        ]
...
class OverallEmotionalWellbeing(Frame):
    ...
    def nextQuestion(self):
        ...
        if answer == '0':
            ...
            OverallEmotionalWellbeing_list.append(answer)
